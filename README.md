# Howdy, I'm Cleburn

  Python developer focused on data analysis and ML, with a background in real estate investing.

  ## Featured Project

  **[Real Estate Investment Analyzer](https://austin-investment-analyzer.streamlit.app)** |
  [Repo](https://github.com/cleburn/austin-investment-analyzer)

  Analyzes 3,000+ neighborhoods across 12 Texas and Florida metros using 25 years of market data. Delivers
   personalized investment recommendations in seconds.

  **Tech:** Python · Pandas · Streamlit

  ---

  ## Tech Stack

  **Languages:** Python · SQL

  **Data:** Pandas · NumPy · Scikit-learn · Matplotlib · Seaborn

  **Tools:** Jupyter · VS Code · Git · Streamlit Cloud

  ---

  ## Background

  U.S. Marine Corps · Professional Athlete (MMA) · VP of Sales · Real Estate Investor

  ---

  ## Contact

  [LinkedIn](https://linkedin.com/in/cleburnwalker) · [Email](mailto:cleburn.walker@gmail.com)
